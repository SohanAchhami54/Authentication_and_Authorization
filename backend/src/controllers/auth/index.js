...
import {createUser, getUserByEmailOrNumber, validateNumber } from "../../services/user/index.js"
...
const SignUp=AsyncError(async (req,res,next)=>{
   ...
   if(existingUser) return next(new Errorhandler('User with this email and phone already exists',400))
    ...
    const user=await createUser({name,email,phone:validNumber,password,verificationMethod})
